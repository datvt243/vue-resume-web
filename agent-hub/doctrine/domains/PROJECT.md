...
| ~~`.eslintrc.cjs` extend `plugin:vue/vue3-essential` nhưng `eslint-plugin-vue` KHÔNG có trong `node_modules`~~ **ĐÃ FIX 2026-08-20** | Trap gốc: `.eslintrc.cjs:2` còn thiếu `@rushstack/eslint-patch` (lỗi thật đầu tiên, chặn trước cả `eslint-plugin-vue`), và `.ts`/`<script lang="ts">` không parse được vì thiếu `parser: '@typescript-eslint/parser'`. Đã cài `@rushstack/eslint-patch` + `eslint-plugin-vue@^9` + `eslint@^8.57.0` (khai báo rõ trong `devDependencies`, trước đây chỉ transitive), thêm `overrides` cho `.ts`/`.vue` trong `.eslintrc.cjs`, thêm script `lint`. | `npm run lint` giờ chạy thật — hiện báo **95 lỗi có thật** (chủ yếu `no-unused-vars`, `vue/multi-word-component-names`) CHƯA được fix, đây là nợ kỹ thuật riêng, không tự động fix hết khi chỉ được giao task "cho lint chạy" |
...
|---|---|---|---|
| 2026-08-20 | Cài `eslint-plugin-vue@^9` (không phải `^10` mới nhất) | `eslint-plugin-vue@10` yêu cầu ESLint 9 + flat config; repo đang ở ESLint 8.57 với `.eslintrc.cjs` (legacy format). `v9` là bản mới nhất còn hỗ trợ `eslint: '^8.0.0'` theo peerDependencies, không cần migrate sang flat config cùng lúc | Cài `v10` + migrate sang `eslint.config.js` flat config luôn — bị từ chối vì đó là thay đổi lớn hơn nhiều so với task được giao ("cài package cho lint chạy"), nên để làm task riêng nếu cần |
| 2026-08-20 | Cài lint deps + fix `.eslintrc.cjs` overrides cho `.ts`/`.vue`, nhưng KHÔNG tự fix 95 lỗi lint phát hiện được | Task được giao là "cho lint chạy thật", không phải "dọn sạch lint" — 95 lỗi là nợ kỹ thuật riêng, phạm vi khác, cần operator quyết định có muốn dọn không | Tự động `eslint --fix` hoặc sửa tay hết 95 lỗi luôn — bị từ chối vì vượt phạm vi task, một số lỗi (`vue/multi-word-component-names`) đổi tên component có thể ảnh hưởng import ở nơi khác |
