...
~~No test suite at all (0 test files, no `test` script)~~ **PARTIALLY FIXED 2026-08-25** | [issue #7](https://github.com/datvt243/vue-resume-web/issues/7) — Vitest installed (`vitest@2.1.9`, pinned below `^3`/`^4` to match the project's `vite@^5.3.1`; `jsdom` pinned to `26.1.0` — `27.x`+ throws `ERR_REQUIRE_ESM` from a transitive `@asamuzakjp/css-color` dependency under this Node/vitest combo). `npm run test` now runs `src/utilities/index.spec.ts` (13 tests, pure functions only). Issue #7 stays OPEN — stores/composables/components (the bulk of its ask, ~60% coverage target) are still untested. | For code inside `src/utilities/`, add/run real Vitest tests. For everything else, still use `npm run build` + manual `npm run dev` check, state clearly this is NOT a real test — see `doctrine/MEMORY.md` → Test row |
